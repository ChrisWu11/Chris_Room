	...
	<view>
		<view class="status-bar">
			...
			<scroll-view scroll-x="true" class="scrollv">
				<view class="users">
					<view v-for="(e,index) in users" :key="index" class="user-l" @tap="otoChat(index)">
...
<script>
	export default {
		...
		onLoad: function (e) {
			...
			this.img = e.img;
			this.uid = e.userId;
			this.join(this.name,this.img,this.uid);
			this.welcome();
			this.myself();
			this.receiveMsg();
			this.getmsg();	
		...
		methods: {
			...
			//加入群
			join: function(name,img,id){
				this.socket.emit('join',name,img,id);
			},
			//欢迎加入群
			welcome: function(){
				//获取即时信息渲染
				this.socket.on('welcome', (name,users) => {
					...
					console.log('welcome--users:',this.users)
				})
			},
			//获取自己
			myself: function(){
				//获取即时信息渲染
				this.socket.on('myself', (name,users) => {
					// console.log(data)
					// console.log('数字'+len)
					this.users = users;
					//插入欢迎信息
					...
					// 	}
					// }
					console.log('myself',this.users)
				})
			},
			//退出群提醒
			quit: function(){
				//获取即时信息渲染
				this.socket.on('quit', (users,myself) => {
					console.log('quit',users,myself)
					//插入退出信息
					let wel = {
						name:myself.name+' 退出群聊',
						id:3,
					}
					this.chat.push(wel);
					//this.member = len;
